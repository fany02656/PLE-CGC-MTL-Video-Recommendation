...
class OMoE(Layer):
    """
    One-gate Mixture-of-Experts model.
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """
         Method for instantiating OMoE layer.
        ...
        super(OMoE, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.gate_kernels = self.add_weight(
            name='gate_kernel_task_{}'.format(1),
        ...
        super(OMoE, self).build(input_shape)
    ...
    def call(self, inputs, **kwargs):
        ...
        # omoe 这里也只是计算一个output
        # g^{k}(x) = activation(W_{gk} * x + b), where activation is softmax according to the paper
        gate_output = K.dot(x=inputs, y=self.gate_kernels)
            # Add the bias term to the gate weights if necessary
        if self.use_gate_bias:
            gate_output = K.bias_add(x=gate_output, bias=self.gate_bias)
        gate_output = self.gate_activation(gate_output)
        # gate_outputs = gate_output  # omoe 这个没有用到

        # omoe 这里最终只得出一个输出（mmoe中是每个expert有一个输出。）
        ...
        final_outputs = K.sum(weighted_expert_output, axis=2)

        return final_outputs

    def compute_output_shape(self, input_shape):
        """
        Method for computing the output shape of the OMoE layer.
    ...
    def get_config(self):
        """
        Method for returning the configuration of the OMoE layer.
        ...
        base_config = super(OMoE, self).get_config()
